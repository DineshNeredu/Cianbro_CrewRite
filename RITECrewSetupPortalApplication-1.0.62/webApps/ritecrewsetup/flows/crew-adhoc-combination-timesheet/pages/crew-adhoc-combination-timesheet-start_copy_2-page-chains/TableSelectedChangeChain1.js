define([
  ...
  class TableSelectedChangeChain1 extends ActionChain {
    ...
    async run(context, { event, previousValue, value, updatedFrom, keys, selected }) {
      const { $page, $flow, $application, $constants, $variables, $functions, $JSON } = context;

      const selectedDataFunc = await $functions.selectedData(selected, JSON.stringify($variables.mainTimeEntriesTableADP.data), keys);

      $variables.selectedData = selectedDataFunc;
