  ...
  class SubmitButtonActionChain extends ActionChain {
    ...
    async run(context, { event, originalEvent }) {
      ...
      const results = await ActionUtils.forEach($variables.selectedData, async (item, index) => {
        ...
        const dateFormatter = await $functions.dateFormatter(item.week_start_date, item.week_end_date, $variables.headerobj.crewDate, $variables.headerobj.crewDate);

        const payloadData = await this.generatePayloadFunc(context, { data: item, user: $application.user.email, startdate: dateFormatter.startDate, endDate: dateFormatter.endDate, daterange: null, crewDate: $variables.headerobj.crewDate, weekid: $variables.headerobj.maxweekid, date: dateFormatter.date });
     ...
     * @param {Object} context
     */
    async generatePayloadFunc(context, { data, user, startdate, endDate, daterange, crewDate, weekid ,date }) {
