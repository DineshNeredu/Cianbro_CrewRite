      ...
      if (current.isNew === 'Y') {
        ...
        const response = await Actions.callRest(context, {
          ...
          body: {
            ...
            'p_combination_id': current.combination_id,
            'p_combination_name': current.combination_name,
            'p_project': current.project_name,
            'p_task_date': current.task_date,
            'p_shift': current.shift,
            'p_location_tax_jurisdiction': current.emp_location,
            'p_work_package': current.work_package,
            'p_department': current.department,
            'p_cost_category': current.cost_center,
            'p_time_type': current.time_type,
            'p_hours': current.hours,
            'p_crewsetup_id': $variables.headerobj.setupId,
            'p_crew_name': $variables.headerobj.crewname,
            'p_project_id': current.project_id,
            'p_project_number': current.project_number,
            'p_task_id': current.task_id,
            'p_task_number': current.task_number,
            'p_task_name': current.task_name,
            'p_end_date': $functions.formatDate(current.end_date),
            'p_start_date': $functions.formatDate(current.start_date),
            'p_crew_date': $functions.formatDate($variables.headerobj.crewDate)
          },
        });
      }
      ...
      mainTableADP.forEach(element => {
        if (element.combination_id === currentRowobj.combination_id) {
          let loopTImeEntryObj = {
            ...
            "time_entry_id": element.resource_id + element.combination_id + element.crewsetup_line_id
          };
