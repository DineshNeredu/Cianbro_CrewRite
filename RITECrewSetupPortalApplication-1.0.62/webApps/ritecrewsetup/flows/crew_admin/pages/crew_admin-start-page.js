define([], () => {
  ...
  PageModule.prototype.getSyncValue = function(data){
    if(data){
      let syncValue=[];
       data.forEach((key)=>{
        syncValue.push(key);
  ...
  PageModule.prototype.getUniqueCrewName= function(data){
    if(data){
      let uniqCrew=[];
      let crewNames=[];
      data.forEach((itm)=>{
        if(crewNames.indexOf(itm.crew_name)===-1){
          crewNames.push(itm.crew_name);
          uniqCrew.push(itm);
        }
      });
      return uniqCrew;
   ...
   PageModule.prototype.getUniqueSuperviosr= function(data){
    if(data){
      let uniqSuperviosr=[];
      let supervisor=[];
      data.forEach((itm)=>{
        if(supervisor.indexOf(itm.supervisor)===-1){
          supervisor.push(itm.supervisor);
          uniqSuperviosr.push(itm);
        }
      });
      return uniqSuperviosr;
    }
  };
  ...
  PageModule.prototype.insertAll = function(data){
    if(data){
      let lookups=[];
      lookups.push({"lookup_code":"All"});
      data.forEach((itm)=>{
        lookups.push(itm);
      });
      return lookups;
