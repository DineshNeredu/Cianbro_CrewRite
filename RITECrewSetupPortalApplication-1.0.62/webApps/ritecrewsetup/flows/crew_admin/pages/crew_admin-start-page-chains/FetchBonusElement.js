...
const response6 = await Actions.callRest(context, {
        endpoint: 'CrewRite_ORDS/getCrewRite_CrewDetailsSearch',
      });
      if (response6.ok) {
        const crewDtls = await $functions.getUniqueCrewName(response6.body.items);

        const superDtls = await $functions.getUniqueSuperviosr(response6.body.items);
        $variables.crewADP.data = crewDtls;
         $variables.moniter.crew_name = crewDtls[0].crew_name;
        $variables.supervisorADP.data = superDtls;

       const dateRangeDtls = await $application.functions.populateDateRangeJS();
     // console.log(dateRangeDtls)

       $variables.dataRangeADP.data= dateRangeDtls.dateRange;
       $variables.moniter.week = dateRangeDtls.week;
      ...
      if(response7.ok){
        $variables.dashObj = response7.body.items[0];
      }
      ...
      const response5 = await Actions.callRest(context, {
        endpoint: 'CrewRite_ORDS/getCrewRite_IntegrationObjects',
      });
      if(response5.ok){        
        const payload = await $functions.insertAll(response5.body.items);
        $variables.SyncFieldsADP.data = payload;
      }
      const response4 = await Actions.callRest(context, {
        endpoint: 'CrewRite_ORDS/getCrewRite_CustomerDetails',
      });
      if(response4.ok){
        $variables.customerADP.data = response4.body.items;
      }     
    ...
    //  });
      ...
      $variables.crewApplicationControlObj.p_bonus_hour = response3.body.items[0].bonus_hours;
