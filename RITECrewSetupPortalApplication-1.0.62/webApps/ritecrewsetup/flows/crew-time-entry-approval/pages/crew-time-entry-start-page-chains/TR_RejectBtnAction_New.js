  ...
  class TR_RejectBtnAction_New extends ActionChain {
    ...
    async run(context) {
     ...
     // if ($page.variables.searchobj.specific === "DAY") {
         let dateFormatter ={};
        if ($variables.searchobj.crewdate) {
           dateFormatter = await $functions.dateFormatter(undefined, undefined, $variables.searchobj.crewdate);
        }
        ...
        if (isSuccess) {
          await Actions.fireNotificationEvent(context, {
           ...
           await Actions.fireNotificationEvent(context, {
            summary: 'TimeSheet Rejected Failed',
            type: 'error',
            displayMode: 'transient',
          });
        }
