define([
  ...
  const validateGroup =
    await $application.functions.validateGroup('SearchValidationgroup');
  ...
  if ($page.variables.searchobj.specific === "DAY") {
    dateFormatter = await $functions.dateFormatter(
      undefined,
      undefined,
      $variables.searchobj.crewdate
    );

    const days = await $functions.getDay(
  ...
  const response = await Actions.callRest(context, {
    ...
    uriParams: {
      contract_number: $variables.searchobj.contract || "",
      crew_name: $variables.searchobj.timecrew || "",
      customer_name: "",
      project_number: $variables.searchobj.project || "",
      search_date:
        $variables.searchobj.specific === "DAY"
          ? dateFormatter?.crewDate || ""
          : $variables.searchobj.specific === "WEEK"
