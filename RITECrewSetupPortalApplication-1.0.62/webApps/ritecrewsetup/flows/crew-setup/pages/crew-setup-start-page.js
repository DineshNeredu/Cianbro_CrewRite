define([], () => {
  ...
  class PageModule {
    ...
    getBetweenWeekdays(eindx){
      let weeks =['Monday','Tuesday','Wednesday','Thursday','Friday','Saturday','Sunday'];
      let edix = weeks.indexOf(eindx);
      return  weeks.slice(0, edix + 1);
    };
    ...
    createPutPayload(linesObj) {
      ...
      let payload = {
        "p_project_id": linesObj.project_id,
        "p_project_number": linesObj.project_number,
        "p_project_name": linesObj.project_name,
        "p_task_id": linesObj.task_id,
        "p_task_number": linesObj.task_number,
        "p_task_name": linesObj.task_name,
        "p_craft_override": linesObj.craft_override,
        "p_effective_start_date": formatDate(linesObj.effective_start_date_copy),
        "p_effective_end_date": formatDate(linesObj.effective_end_date_copy),
        "p_per_diem_amount": linesObj.perdiem_flag === true ? linesObj.per_diem_amount : 0,
        "p_per_diem_rate":  linesObj.per_diem_rate,
        "p_bonus_amount": linesObj.bonus_flag === true ? linesObj.bonus_amount : 0,
        "p_bonus_rate": linesObj.bonus_rate,
        "p_perdiem_flag": linesObj.perdiem_flag,
        "p_bonus_flag": linesObj.bonus_flag,
        "p_bonus_quantity": linesObj.bonus_flag === true ? linesObj.bonus_quantity : 0,
        "p_perdiem_quantity": linesObj.perdiem_flag === true ? linesObj.perdiem_quantity : 0,
        "p_ot_threshold_limit": linesObj.ot_threshold_limit,
        "p_expenditure_type_name": linesObj.expenditure_type_name,
        "p_expenditure_type_id": linesObj.expenditure_type_id,
        "p_system_linkage_function": linesObj.system_linkage_function,
        "p_mon_in_time": linesObj.mon_in_time,
        "p_mon_out_time": linesObj.mon_out_time,
        "p_tue_in_time": linesObj.tue_in_time,
        "p_tue_out_time": linesObj.tue_out_time,
        "p_wed_in_time": linesObj.wed_in_time,
        "p_wed_out_time": linesObj.wed_out_time,
        "p_thu_in_time": linesObj.thu_in_time,
        "p_thu_out_time": linesObj.thu_out_time,
        "p_fri_in_time": linesObj.fri_in_time,
        "p_fri_out_time": linesObj.fri_out_time,
        "p_sat_in_time": linesObj.sat_in_time,
        "p_sat_out_time": linesObj.sat_out_time,
        "p_sun_in_time": linesObj.sun_in_time,
        "p_sun_out_time": linesObj.sun_out_time,
        "p_sun_total_hours": linesObj.sun_quantity,
        "p_mon_total_hours": linesObj.mon_quantity,
        "p_tue_total_hours": linesObj.tue_quantity,
        "p_wed_total_hours": linesObj.wed_quantity,
        "p_thu_total_hours": linesObj.thu_quantity,
        "p_fri_total_hours": linesObj.fri_quantity,
        "p_sat_total_hours": linesObj.sat_quantity,
        "p_crewsetup_line_id":linesObj.crewsetup_line_id
      };
      return payload;
    ...
    createPostPayload(linesObj, dialogueLabel, headerObj, email) {
      ...
      let payload = {
        ...
        system_linkage_function: linesObj.system_linkage_function ? linesObj.system_linkage_function : '',
...
PageModule.prototype.getUniqueResources = function (data,rsnmae) {
if(data){
let resourceArray=[];
  data.forEach((itm) => {
    if (itm.resource_name === rsnmae) {
      resourceArray.push(itm);
    }
  });
  ...
  PageModule.prototype.getUniqueExpeditures = function (data) {
    if (data) {
      debugger;
      let obj = { body: { items: [] } };
      let uniqCust = [];
      data.items.forEach((itm) => {
        if (uniqCust.indexOf(itm.expenditure_type_name) === -1) {
          uniqCust.push(itm.expenditure_type_name);
          obj.body.items.push(itm);
        }
      });
      return obj;
  ...
  PageModule.prototype.getPersonNameNew = function (data, pid,sid,suvisor) {
    if (data) {
      let obj={};
      for (let itm of data) {
        if (itm.person_id === pid) {
          obj.primary_name = itm.person_name;
        }
         if (itm.person_id === sid) {
          obj.second_name = itm.person_name;
        }
          if (itm.person_id === suvisor) {
          obj.super_name = itm.person_name;
        }
      }
      return obj;
    }
