      ...
      if (validate === 'valid') {
        ...
        if (timeValidator.isValid) {
          const createPostPayload = await $functions.createPostPayload($variables.linesObj, $variables.dialogueLabel, $variables.crewSetupHeaderObj, $application.user.email);
            let response={};
          if ($variables.dialogueLabel==="Assign") {
             response = await Actions.callRest(context, {
              endpoint: 'CrewRite_ORDS/postCrewRite_CrewSetupLines',
              body: createPostPayload,
            });
          }
          else if($variables.dialogueLabel==="Edit"){
            const createPutPayload = await $functions.createPutPayload($variables.linesObj);
             response = await Actions.callRest(context, {
              endpoint: 'CrewRite_ORDS/putCrewRite_CrewSetupLines',
              body: createPutPayload,
            });
